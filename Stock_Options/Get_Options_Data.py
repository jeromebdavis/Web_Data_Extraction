...
#Set sleep time for loops
sleep_time_inner_loop = 5
sleep_time_outer_loop = 120
...
#Date of interest
Date_Interest = datetime(2021, 3, 19)
...
for TickerL in ticker_list: 
    ...
    for Ticker in TickerL:
        ...
        query_string = 'https://finance.yahoo.com/quote/'+Ticker+'/options?date='+str(date_integer)
        print(query_string)
        driver.get(query_string)
        time.sleep(sleep_time_inner_loop)
        res = driver.page_source
        bs4Soup = bs4.BeautifulSoup(res, 'html.parser')
        
        #Get ticker price. Go to next Ticker if gives error
        try:
        ...
        for i in data_list[1:]:
            if not i:
                ...
                result[-1].append(i)
        ##Check if options data read in correctly. If not move to next ticker 
        if(len(result[0][0])<11):
            rows_read = str(len(result[0][0]))
            print(Ticker + " call options read error. Only " + rows_read + " rows read.")                    
            continue
        if(len(result[1][0])<11):
            rows_read = str(len(result[1][0]))
            print(Ticker + " put options read error. Only " + rows_read + " rows read.")                    
            continue

        #Process and perform calculations on queried call option data
        ...
        df_call['Type'] = 'Call'
        ##Remove "-" from numeric columns and replace with '0.00'
        df_call[['Bid','Ask','Strike','Last_Price','Ticker_Price']] = (
            df_call[['Bid','Ask','Strike','Last_Price','Ticker_Price']].replace(['-'], ['0.00']))
        ##Remove "," from all columns
        df_call = df_call.apply(lambda x: x.str.replace(',',''))
        ...
        df_put[['Bid','Ask','Strike','Last_Price','Ticker_Price']] = (
            df_put[['Bid','Ask','Strike','Last_Price','Ticker_Price']].replace(['-'], ['0.00']))
        ##Remove "," from all columns
        df_put = df_put.apply(lambda x: x.str.replace(',',''))
        ##Convert numeric columns to numeric type
        df_put[['Bid','Ask','Strike','Last_Price','Ticker_Price']] = (
        ...
        #Put extra time to not overload yahoo scraping        
        time.sleep(sleep_time_inner_loop)

    #Put extra time as yahoo only takes around 30 scrapes at a time
    time.sleep(sleep_time_outer_loop)        
